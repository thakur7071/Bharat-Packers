...
function Hero() {
  ...
  return (
    <div className="relative w-full min-h-[60vh] sm:min-h-[70vh] md:min-h-[80vh] lg:min-h-[90vh] xl:min-h-screen mb-10 overflow-hidden">
      <img
        src={card.image}
        alt={card.title}
        className="w-full h-full object-cover transition-opacity duration-700 ease-in-out"
        key={currentIndex} // helps React re-render for animation if you add it later
      />

      {/* Overlay Card */}
      <div className="absolute inset-0 flex items-center justify-center px-4">
        <div
          className="bg-white/30 backdrop-blur-sm p-6 sm:p-8 md:p-10 rounded-xl shadow-md max-w-full sm:max-w-md md:max-w-lg lg:max-w-xl text-center
            w-full sm:w-auto
          "
          style={{ maxHeight: '80vh', overflowY: 'auto' }}
        >
          <h1 className="text-lg sm:text-2xl md:text-4xl font-bold text-[#0047AB] mb-4">
            {card.title}
          </h1>
          <p className="text-xs sm:text-sm md:text-base text-[#0047AB] mb-6">
            {card.description}
          </p>
          <button
            className="cursor-pointer bg-[#0047AB] text-white px-6 py-2 sm:px-8 sm:py-3 rounded-full font-semibold
              hover:bg-[#003680] focus:outline-none focus:ring-4 focus:ring-[#0047AB]/60 transition"
          >
            {card.buttonText}
          </button>
        </div>
